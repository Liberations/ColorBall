# BallConnect - Privacy Policy & User Agreement

---

## 中文版

### 隐私政策

**生效日期：2026-03-03**

1. 本应用为单机小游戏，不收集、存储或共享任何个人信息。

2. 本应用不请求、不使用任何设备权限（如存储、摄像头、定位、通讯录等）。

3. 本应用集成了 Google Mobile Ads 广告服务。广告由 Google 提供，Google 可能会根据其隐私政策收集设备信息用于广告投放。详情请参阅 [Google 隐私政策](https://policies.google.com/privacy)。

4. 本应用不使用 Cookies，也不进行跨站或跨应用跟踪。

5. 未成年人政策：应用不收集任何信息，无需提供监护人同意。广告服务由 Google 提供，请参阅 Google 的未成年人隐私政策。

6. **变更**：如未来功能变更涉及隐私数据或权限，将在更新前于此政策中明确说明并征求同意。

7. **联系方式**：如对本隐私政策有任何疑问，请通过应用商店开发者联系方式与我们取得联系。

---

### 用户协议

**最后更新日期：2024年1月**

欢迎使用连线球球！在使用本应用前，请仔细阅读以下条款。

#### 1. 服务说明

连线球球是一款休闲益智游戏，提供连线解谜的游戏体验。

#### 2. 用户账户

- 本应用不需要注册账户即可使用
- 游戏数据存储在本地设备上
- 如更换设备或卸载应用，游戏数据可能会丢失

#### 3. 虚拟物品

- 本应用包含虚拟货币（星星、钻石）和虚拟道具（跳过卡、重试卡、抽卡券、卡片等）
- 所有虚拟物品仅限在本应用内使用，不具有现实货币价值
- 虚拟物品不可转让、兑换或退款

#### 4. 广告服务

- 本应用集成了 Google Mobile Ads 广告服务
- 观看广告可获得游戏内奖励
- 广告内容由 Google 提供，我们不对广告内容负责
- Google 可能会根据其隐私政策收集设备信息

#### 5. 用户行为规范

用户不得：
- 使用外挂、作弊软件或其他非法手段获取游戏优势
- 尝试破解、反编译或修改应用
- 利用漏洞获取不正当利益
- 干扰其他用户的游戏体验

#### 6. 知识产权

- 本应用的所有内容（包括但不限于图像、音效、代码、设计）均受版权保护
- 未经授权，不得复制、修改、传播本应用的任何内容

#### 7. 免责声明

- 本应用按"现状"提供服务，不提供任何形式的保证
- 我们不对因使用本应用而产生的任何损失负责
- 我们保留随时修改或终止服务的权利

#### 8. 协议变更

我们保留随时修改本协议的权利。修改后的协议将在应用内公布，继续使用本应用即表示您接受修改后的条款。

#### 9. 适用法律

本协议的解释和执行适用中华人民共和国法律。

---

## English Version

### Privacy Policy

**Effective Date: 2026-03-03**

1. This application is an offline game that does not collect, store, or share any personal information.

2. This application does not request or use any device permissions (such as storage, camera, location, contacts, etc.).

3. This application integrates Google Mobile Ads advertising service. Ads are provided by Google, which may collect device information for ad delivery according to its privacy policy. Please refer to [Google Privacy Policy](https://policies.google.com/privacy) for details.

4. This application does not use Cookies or engage in cross-site or cross-application tracking.

5. Children's Policy: The application does not collect any information, so no parental consent is required. Advertising services are provided by Google, please refer to Google's children's privacy policy.

6. **Changes**: If future feature changes involve privacy data or permissions, they will be clearly stated in this policy before the update and consent will be sought.

7. **Contact**: If you have any questions about this privacy policy, please contact us through the developer contact information on the app store.

---

### User Agreement

**Last Updated: January 2024**

Welcome to BallConnect! Please read the following terms carefully before using the App.

#### 1. Service Description

BallConnect is a casual puzzle game that provides a line-connecting puzzle experience.

#### 2. User Account

- The App can be used without registering an account
- Game data is stored locally on the device
- Game data may be lost if you change devices or uninstall the App

#### 3. Virtual Items

- The App contains virtual currencies (stars, diamonds) and virtual items (skip cards, retry cards, draw tickets, cards, etc.)
- All virtual items are for use within the App only and have no real monetary value
- Virtual items cannot be transferred, exchanged, or refunded

#### 4. Advertising Services

- The App integrates Google Mobile Ads advertising service
- Watching ads can earn in-game rewards
- Ad content is provided by Google, we are not responsible for ad content
- Google may collect device information according to its privacy policy

#### 5. User Conduct

Users must not:
- Use cheats, hacking software, or other illegal means to gain game advantages
- Attempt to crack, decompile, or modify the App
- Exploit vulnerabilities for unfair benefits
- Interfere with other users' gaming experience

#### 6. Intellectual Property

- All content in the App (including but not limited to images, sound effects, code, design) is copyright protected
- No content of the App may be copied, modified, or distributed without authorization

#### 7. Disclaimer

- The App is provided "as is" without any form of warranty
- We are not responsible for any losses arising from the use of the App
- We reserve the right to modify or terminate services at any time

#### 8. Agreement Changes

We reserve the right to modify this Agreement at any time. Modified agreements will be published within the App, continued use of the App indicates your acceptance of the modified terms.

#### 9. Applicable Law

The interpretation and enforcement of this Agreement shall be governed by the laws of the People's Republic of China.

---

## Contact / 联系方式

If you have any questions about this Privacy Policy or User Agreement, please contact us through the app store developer contact information.

如果您对本隐私政策或用户协议有任何疑问，请通过应用商店开发者联系方式联系我们。

---

**BallConnect Team / 连线球球团队**
